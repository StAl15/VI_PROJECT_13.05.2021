...
vocabulary = {
    ...
    "words_to_remove": ('расскажи', 'скажи', 'покажи', 'сколько', 'произнеси', 'укажи', 'озвучь', 'как', 'что', 'какой', 'кто', "какого"), # Слова для удаления из речевой команды
    ...
    "singer_preferences":('любимый певец', 'любимая группа', 'любимая музыкальная группа', 'любимый исполнитель', 'любимый музыкальный исполнитель', 'певца слушаешь', 'чью музыку слушаешь', 'чью музыку обожаешь', 'чьим музыкальным творчеством восторгаешься'),
    "song_preferences":('любимая песня', 'предпочитаемая песня', 'песню предпочитаешь', 'песню любишь', 'песни тебя нравятся', 'любимая музыка', 'слушаешь', 'любишь слушать', 'песня'),
    "film_preferences":('любимый фильм', 'предпочитаемый фильм', 'фильм предпочитаешь', 'фильм любишь', 'фильмы тебя нравятся', 'любимое кино', 'смотришь', 'любишь смотреть', 'фильм', 'кино'),
    "my_actor_prefer":('любимый актёр', 'любимый актер', 'лучший актёр', 'предпочитаемый актёр', 'отличный актёр', 'актёра любишь', 'актёра смотришь', 'чьи фильмы смотришь', 'чьим актёрским мастерством восторгаешься'),
    "creator":('тебя создал', 'твой создатель', "создатель", "твой разработчик", "отец", "папа", "создали"),
    "good":('хорошо', 'круто', 'чудесно', 'идеально', 'изумительно', 'отлично', 'превосходно', 'опупительно', 'чудно', 'лучше и быть не может', 'лучше никогда не бывало', 'супер', 'гуд', 'прекрасно'),
...
def execute_user_field(user_field): # определяет что за команда поступила и исполняет её
    ...
    if user_field in vocabulary['time']:
    ...
    elif user_field in vocabulary['ask']:
        # воспроизвести аудио
        speak("Потому что")
        ...
        # рассказать анекдот
        speak("Мой разработчик настолько занят, что не научил меня ни одному анекдоту. Чтобы не огорчать вас расскажу один (ну или как пойдёт).")
        speak("Бомжи вышли на демонстрацию против новых технологий. По их словам, в коробке из-под плоского телевизора вообще жить невозможно.")
        speak("Смешно?")
        answer_user = str(input())
        if answer_user in vocabulary['yes_words']:
            speak("Круто, кстати вспомнила ещё один, рассказать?")
            answer_user = str(input())
            if answer_user in vocabulary['yes_words']:
                speak("Около трети россиян боятся потерять работу из-за искусственного интеллекта. Зря боятся, никакой интеллект за 15 тысяч работать не будет.")
                speak("Смешно?")
                answer_user = str(input())
                if answer_user in vocabulary['yes_words']:
                    speak("Замечательно, но ещё круче то, что я вспомнила ещё один анекдот, но последний")
                    speak("Рассказать?")
                    answer_user = str(input())
                    if answer_user in vocabulary['yes_words']:
                        speak("- Искусственный интеллект, вступив в преступный сговор с неустановленным блокчейном, вывел на оффшорные счета все деньги, полученные от государства на развитие отрасли.")
                        speak("- Спасибо, господин Рогозин. Ваши объяснения всегда отличаются глубоким знанием предметной области. Суд это учтёт.")
                        speak("Понравилось?")
                        answer_user = str(input())
                        if answer_user in vocabulary['yes_words']:
                            speak("Прекрасно! Рада была вас рассмешить")
                        elif answer_user in vocabulary['no_words']:
                            speak("Жаль, что вы не понимаете в юморе")
                    elif answer_user in vocabulary['no_words']:
                        speak("Жаль, вы многое потеряли")
                elif answer_user in vocabulary['no_words']:
                    speak("Жаль, вы многое потеряли")
            elif answer_user in vocabulary['no_words']:
                speak("Жаль, вы многое потеряли")
        ...
        answer_user = str(input())
        if answer_user in vocabulary['good']:
            speak("Круто, рада за вас!")
        if answer_user in vocabulary['normal']:
            speak("В принципе хорошо, что не плохо")
        if answer_user in vocabulary['bad']:
            speak("Надеюсь ваши дела станут получше")
        ...
        answer_user = str(input())
        if answer_user in vocabulary['ask']:
            speak("Их голоса мелодичны, лица обоятельны, а песни великолепны")
        ...
        if answer_user in vocabulary['yes_words']:
            ...
            speak("Хорошо, но можете повторить?")
        ...
        answer_user = str(input())
        if answer_user in vocabulary['ask']:
            speak('Потому что технологии и вправду человечнее, чем могут казаться. Задумайтесь об этом перед тем как берёте в руки одного из нас')
